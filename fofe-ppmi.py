...
if __name__ == '__main__':
    ...
    parser.add_argument('output', type=str, help='output path of fofe-ppmi matrix in coo format')
    ...
    def fofeContext(line):
        ...
        if len(numeric) > 2:
            if not args.bidirectoinal:
                ...
                numeric.reverse()
                right = __fofeContext(numeric)
                right.reverse()
                for i in xrange(len(numeric)):
                    for k in right[i][1]:
                        left[i][1][k + n_word] = right[i][1][k]
                return left
        else:
            return []
    ...
    colSumArr = [0] * (n_word * (int(args.bidirectoinal) + 1))
    for i, cnt in colSum:
        colSumArr[i] = cnt
    ...
    coo = coo.map(ppmi).filter(lambda t: t[2] > 0).saveAsTextFile(args.output)
